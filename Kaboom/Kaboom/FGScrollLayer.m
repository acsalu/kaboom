...
//        int visibleBoundUp = currentPos.y / pageHeight_ ;
        int visibleBoundUp = (currentPos.y-100) / 150 ;
        ...
        if (visibleBoundUp < [layers_ count]) {
            ...
            int visibleBoundDown = (currentPos.y) / 130+1;
...
//            NSLog(@"y: %f, visibleBoundUp: %d, visibleBoundDown: %d",currentPos.y,visibleBoundUp,visibleBoundDown);

        }

        currentPage = [self pageNumberForPosition:currentPos];
        //[self scaleAllLayer:currentPos];
    }
...
- (void)ccTouchMoved:(UITouch *)touch withEvent:(UIEvent *)event
{
	...
	if ( (state_ != kFGScrollLayerStateSliding) && (fabsf(touchPoint.y-startSwipe_) >= self.minimumTouchLengthToSlide) )
    {
...
{
	CGFloat pageFloat = self.position.y / (pageHeight_ - self.pagesOffset)-1;
    
//    NSLog(@"y=%f",self.position.y);
    
	int pageNumber = ceilf(pageFloat);
//	if ( (CGFloat)pageNumber - pageFloat  >= 0.1f)
//		pageNumber--;
...
- (void)ccTouchEnded:(UITouch *)touch withEvent:(UIEvent *)event
{
    ...
    if(!isMove){
        CGPoint touchPoint = [touch locationInView:[touch view]];
        touchPoint = [[CCDirector sharedDirector] convertToGL:touchPoint];
        if((touchPoint.y-startPoint_)<10){
            CGSize screenSize = [CCDirector sharedDirector].winSize;
            int delta = (touchPoint.y-screenSize.height/2)/(pageHeight_/4+25);
...
-(void)scaleLayer:(CCLayer *)layer withScale:(float)newScale scaleCenter:(CGPoint) scaleCenter{
//    CGPoint oldCenterPoint = ccp(scaleCenter.x * layer.scale, scaleCenter.y * layer.scale);
//    [layer setScale:newScale];
    layer.scale = newScale;
//    CGPoint newCenterPoint = ccp(scaleCenter.x * layer.scale, scaleCenter.y * layer.scale);
//    CGPoint centerPointDelta  = ccpSub(oldCenterPoint, newCenterPoint);
//    layer.position = ccpAdd(layer.position, centerPointDelta);
}

-(void)scaleAllLayer:(CGPoint) scaleCenter{
    //NSLog(@"page: %d",currentPage);
    [self scaleLayer:[layers_ objectAtIndex:currentPage] withScale:1.2f scaleCenter:scaleCenter];
    
    if(currentPage>0){
        [self scaleLayer:[layers_ objectAtIndex:currentPage-1] withScale:1.0f scaleCenter:scaleCenter] ;
    }
    if(currentPage+1<self.totalPagesCount)
        [self scaleLayer:[layers_ objectAtIndex:currentPage+1] withScale:1.0f scaleCenter:scaleCenter];
    
}

-(void) zooming:(CCLayer*)layer{
